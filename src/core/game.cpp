...
class Game {
    ...
    //  Will be implemented with UI library.
    // core::Screen* screen;
    ...
    int run() {
        ...
        this->score = 0;
        this->arena = new core::Arena();
        // this->screen = new core::Screen();
        //  The main loop.
        bool gameOver = false;
        while (!gameOver) {
            runTick();
...
// The following code originally came from Scevenate. They are now ported to their separate files.
// Their comments are kept for reference.
